...
/// Follows exact same pattern as FamiliesListPage with BaseListPageLogic.
/// </summary>
public partial class VariantsListPage : ContentPage
{
    private readonly BaseListPageLogic<Models.Variant, VariantItemViewModel> _base;

    /// <summary>
    /// Initialize the variants list page with dependency injection and composition
    /// </summary>
    public VariantsListPage(VariantsListViewModel viewModel)
    {
        InitializeComponent();

        // TRUE composition - use the base logic directly
        _base = new BaseListPageLogic<Models.Variant, VariantItemViewModel>(viewModel);
        _base.SetupPage(this, VariantListView, RootGrid, FabButton, ListRefresh);

        this.LogInfo("🚀 REFACTORED VariantsListPage - using BaseListPageLogic composition");
    }
